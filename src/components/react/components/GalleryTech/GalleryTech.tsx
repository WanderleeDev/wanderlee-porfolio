...
const GalleryTech = (): JSX.Element => {
  const { gallery: {tech, title} } = useContext(TechContext)
  const [valueScroll, setValueScroll] = useState<IMarqueeData>({ name: '', color: '#fff' })
      ...
      <div className='gallery-icons'>
        {
          tech.map((t, i) =>
            <img
              key={i}
              onMouseEnter={() => { updateScroll(t.name, t.color) }}
              style={{ color: t.color }}
              width={32}
              height={32}
